...
{
    return (findFaultyNodeUtil(info_, left, right, root, -10000, 10000));
}
...
int findFaultyNodeUtil(int info_[], int left[], int right[], int root, int min, int max)  
{
    ...
    {
        int depth = floor(log(root+1)/log(2));
        printf("\nNode %d at depth %d, breaks the BST.\n", info_[root], depth);
        return info_[root];
    }  
    ...
    if(left[root]!=-1 && right[root]!=-1)
        ...
        return findFaultyNodeUtil(info_, left, right, left[root], min, info_[root]-1);
    else if (left[root]==-1 && right[root]!=-1)
        return findFaultyNodeUtil(info_, left, right, right[root], info_[root]+1, max); 
}  
...
int main()
{
    ...
    else if (yes == 0)
    {
        ...
        printf("\nIf you see multiple nodes breaking the BST, then the first node that is printed to break the BST is the node at the minimum depth\n\n");
    }
}